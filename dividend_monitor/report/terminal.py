...
格式：字符分隔线 + 各指数估值区块 + 周 KDJ 行 + 动态仓位建议区块。
估值数据缺失时仍展示 KDJ（两个数据模块解耦，互不依赖）。
"""
...
def print_report(
    ...
    for idx, res in zip(INDEXES, val_results):
        ...
        if "error" in res:
            ...
            src_tag = "  ⚠ 妙想数据" if res.get("source") == "mx" else "  [缓存]"
            ...
            if hist_start:
                hist_tag = f"{hist_start} 至今"
                hist_tag += f"（约 {hist_years:.1f} 年，{hist_n} 个交易日）" if hist_years else f"（{hist_n} 个交易日）"
                print(f"    历史区间  {hist_tag}")
    ...
    print(f"\n{'═' * W}")
    print(f"  ─ 百分位说明：基于妙想API返回历史区间计算；PE%位越低越便宜；股息率%位越高越丰厚")
