...
| 模式 | 参数 | 说明 |
|------|------|------|
| 精简模式 | `--mode compact` | ASCII图表+信号分布图（默认） |
| 经典模式 | `--mode classic` | 完整技术指标表格 |
| 卡片模式 | `--mode card` | 每只ETF卡片式展示 |
| 图表模式 | `--mode chart` | ASCII可视化图表 |
| 雷达模式 | `--mode radar` | 多维度评分对比 |
| 矩阵模式 | `--mode matrix` | 多维度横向对比 |
